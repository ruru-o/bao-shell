...
# -----------------------------------------------------------------------------
# Per-task execution (Animated, Debloated & 100% Stable)
# -----------------------------------------------------------------------------
$SpinnerFrames = @('⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏')
...
$RepoRawBase = 'https://raw.githubusercontent.com/ruru-o/bao-shell-profile/main'
$RepoFiles = @{
...
function Get-Utf8TextFromBytes {
    param([byte[]]$Bytes)
    # GitHub raw files are UTF-8. Decode explicitly so Windows PowerShell 5.1
    # never interprets the Braille/Nerd-Font characters using the ACP.
...
function Install-RepoFiles {
    ...
    foreach ($target in $targets) {
        ...
        Invoke-Task -Label "writing $leaf profile" -Action {
            $enc = New-Object System.Text.UTF8Encoding($true)
            [System.IO.File]::WriteAllText($target, $profileText, $enc)
...
function Revert-TerminalAndConsoleSettings {
    ...
    #revert Win32 Console registry overrides
...
function Test-ExecutionPolicyInstalled {
    try {
        $policy = Get-ExecutionPolicy -Scope CurrentUser -ErrorAction SilentlyContinue
        if ($policy -and $policy -ne 'Restricted' -and $policy -ne 'Undefined') {
            return $true
        }
    } catch {}
    return $false
...
function Read-Menu {
    ...
    $render = {
        for ($i = 0; $i -lt $Items.Count; $i++) {
            $isSel = ($i -eq $selected)
            $isDim = $Disabled[$i]
            $label = if ($isSel) { "$([char]0x276F) $($Items[$i])" } else { "  $($Items[$i])" }
            $tag = $StatusTags[$i]

            $pad = [Math]::Max(0, [Math]::Floor(($width - $label.Length - 2 - $(if ($tag) { $tag.Length + 3 } else { 0 })) / 2))
            $color = if ($isDim) { $grey } elseif ($isSel) { $Accent } else { $Dim }
            $tagColor = if ($isDim) { $greyTag } else { $Dim }

            $line = "$(' ' * $pad)$(Write-Fg $color)$label$Reset"
            if ($tag) { $line += "  $(Write-Fg $tagColor)$([char]0x2022) $tag$Reset" }
            Write-Host $line
        }
        if ($Hint) {
            Write-Host ''
...
function Read-Choice {
    Write-Host ''
    $installed = Test-BaoInstalled
    $items = @('Install', 'Uninstall', 'Cancel')
    ...
    while ($true) {
        try {
            ...
            switch ($choice) {
                ...
                'Cancel'    {
                    Write-Host ''
                    Log 'Cancelled - exiting bao-shell.' $Dim
                    Write-Host ''
                    break
                }
            }
